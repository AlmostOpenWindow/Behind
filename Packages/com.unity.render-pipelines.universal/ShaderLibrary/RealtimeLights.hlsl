...
{
    // DISCLAIMER:
    // These changes were made by ME in a desperate attempt
    // to solve the problem of maximizing the INTENSITY for spotlights
    //
    // the main change:
    // FROM lightAtten = rpc(distanceSqr);
    // TO   lightAtten = rpc(1 + 25 * distanceSqr / lightRangeSqr);
    // So we have dependence from lightRange and not only distance to object; And of course some shitty constants "25"
    // when lightAtten = rpc(1) - it is like directional light and hasn't got attenuation at all
    ...
    // smoothFactor = smoothFactor * smoothFactor;
    // return lightAtten * smoothFactor;
    //____ORIGINAL END____

    //____SOME INSPIRATION FROM THE INTERNET____
    //float lightRangeSqr = rcp(distanceAttenuation.x);
    ...
    // // Therefore:
    // float fadeDistance = (0.8 * 0.8 * lightRangeSqr);
    // float fadeDistanceSqr = fadeDistance * fadeDistance;
    //
    // // We can rewrite that to fit a MAD by doing
    // // distanceSqr * (1.0 / (fadeDistanceSqr - lightRangeSqr)) + (-lightRangeSqr / (fadeDistanceSqr - lightRangeSqr));
    // // distanceSqr *        distanceAttenuation.y            +             distanceAttenuation.z
    //
    // //half smoothFactor = (lightRangeSqr - distanceSqr) / (lightRangeSqr - fadeDistance);
    //
    ...
    // #endif
    //
    // //return rcp(1 + 25 * distanceSqr / lightRangeSqr) * smoothFactor;
    // return lightAtten * smoothFactor;
    //____END OF INSPIRATION FROM INTERNET____
}
